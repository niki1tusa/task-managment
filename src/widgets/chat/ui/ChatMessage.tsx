...
interface Props {
	...
	value: string;
	isFindMessage: boolean;
}

const ChatMessage = forwardRef<HTMLDivElement, Props>(
	({ message, isFirstInGroup, isLastInGroup, value, isFindMessage }, ref) => {
		...
		return (
			<div
				ref={ref}
				className={clsx('flex items-end', isOwnMessage ? 'justify-end' : 'justify-start')}
			>
				<div
					className={clsx(
				...
				>
					...
					<div
						className={clsx('flex min-w-0 flex-col', isOwnMessage ? 'items-end' : 'items-start')}
					>
						{/* time + name owner */}
						{isFirstInGroup && (
